trigger AGS_TradeSecretAttribute_MinNumUpdate on AGS_Trade_Secret_Attribute__c (after update) {    
    ...
    try
    {
        ...
        for(AGS_Trade_Secret_Attribute__c sc: trigger.new)
        {
        ...
        {  
            if(trigger.old[i].ATTRIBUTE_TEXT_gne__c!=trigger.new[i].ATTRIBUTE_TEXT_gne__c)        
            {
                ...
                if(rv!=null)
                {
                    ...
                    fieldNames.add('ATTRIBUTE_TEXT_gne__c');
