trigger gFRS_ShutdownRuleTrigger on gFRS_Shutdown_Rule__c (before insert, before update) {

    if (Trigger.isBefore && (Trigger.isInsert || Trigger.isUpdate)) {
        Map<Id, gFRS_Shutdown_Rule__c> shutdownRuleMap = new Map<Id, gFRS_Shutdown_Rule__c>([SELECT Id, Start_date__c, End_date__c, Funding_type__c FROM gFRS_Shutdown_Rule__c WHERE Active__c = true]);
        for(gFRS_Shutdown_Rule__c sr : Trigger.new) {
            ...
            for(Id key : shutdownRuleMap.keySet()) {
                if(sr.Active__c == true && sr.Id != key) {
                    gFRS_Shutdown_Rule__c existingSR = shutdownRuleMap.get(key);
