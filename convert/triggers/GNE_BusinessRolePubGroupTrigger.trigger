trigger GNE_BusinessRolePubGroupTrigger on GNE_Business_Role_Public_Group__c (before insert, after insert, 
    after update, before delete)
{
    ...
    if (Trigger.isBefore)
    {
        ...
        if (Trigger.isInsert)
        {
            for (GNE_Business_Role_Public_Group__c record : Trigger.new)
            {
                record.Unique_Key__c = record.Business_Role__c + record.Public_Group_Id__c;
            }
        }
        ...
        /*if (Trigger.isDelete)
        {
            for (GNE_Business_Role_Public_Group__c record : Trigger.old)
            {
    ...
    if (Trigger.isAfter)
    {
        ...
        if (Trigger.isInsert)
        {
            ...
            if (activeIds.size() > 0)
            {
                GNE_BusinessRolePubGroupUtils.HandleActivations(activeIds);
            }
        }
        ...
        if (Trigger.isUpdate)
        {
            ...
            {
                GNE_BusinessRolePubGroupUtils.HandleInactivations(inactiveIds);
            }
        }
    }
}