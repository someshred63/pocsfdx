trigger AGS_ACH_MinNumUpdate on AGS_Allocation_gne__c (after update) {
    ...
    try
    {
        ...
        List<Decimal> versionIDcodes=new List<Decimal>();
          
        List<String> fieldNames = new List<String>();
            
        for(AGS_Allocation_gne__c sc: trigger.new)
        {
        ...
        {  
            if(trigger.old[i].Allocation_Comment_gne__c!=trigger.new[i].Allocation_Comment_gne__c ||
              trigger.old[i].Allocation_Method_Description_gne__c !=trigger.new[i].Allocation_Method_Description_gne__c)
