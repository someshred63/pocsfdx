trigger AGS_DCTH_MinNumUpdate on AGS_Descrete_Threshold__c (after update) {
    ...
    try
    {
         ...
         for(AGS_Descrete_Threshold__c sc: trigger.new)
         {
         ...
         {  
             if(trigger.old[i].LIMIT_AMOUNT_gne__c!=trigger.new[i].LIMIT_AMOUNT_gne__c )
             { 
                ...
                if(rv!=null)
                {
                    ...
                    fieldNames.add('LIMIT_AMOUNT_gne__c');
